...
function _tryPunchMob(){
  ...
  if(bestMob){
    bestMob._hitThisSwing=true;
    const isCrit=player.vel.y<0&&!player.onGround&&!player.sneaking&&!waterAtFeet()&&!headInWater();
    hitMob(bestMob,isCrit);
  }
}
function hitMob(mob,isCrit=false){
  ...
  sfxMobHurt(mob.type);
  if(isCrit)spawnCritParticles(mob.pos.x,mob.pos.y+mob.hitH*0.65,mob.pos.z);
  ...
  mob.idleT=0;
  damageMob(mob,isCrit?2.5*1.5:2.5,"player");
}
